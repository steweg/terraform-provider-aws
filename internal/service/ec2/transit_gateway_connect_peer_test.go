		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "inside_cidr_blocks", "[169.254.200.0/29]"),
					resource.TestCheckResourceAttr(resourceName, "peer_address", "2.2.2.2"),
					resource.TestCheckResourceAttr(resourceName, "tags.%", "0"),
					resource.TestCheckResourceAttr(resourceName, "transit_gateway_address", "1.1.1.1"),
...
func testAccTransitGatewayConnectPeer_InsiteCidrBlocks(t *testing.T) {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckTransitGatewayConnectPeerExists(resourceName, &transitGatewayConnectPeer1),
					resource.TestCheckResourceAttr(resourceName, "transit_gateway_address", "3.3.3.3"),
				),
			},
		},
	})
...
resource "aws_ec2_transit_gateway_connect_peer" "test" {
  inside_cidr_blocks = ["169.254.200.0/29", "169.254.201.0/29]
  peer_address = "1.1.1.1"
  transit_gateway_attachment_id = aws_ec2_transit_gateway_connect.test.id
}
`)
